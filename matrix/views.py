...
class RoomJoinView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for room in models.ManagedRoom.objects.all():
            rooms.append({
                'name': room.name,
                'id': room.pk,
                'invited': models.UserInvitation.objects.filter(user=matrix_user, room=room).exists()
            })
    ...
    def dispatch(self, request, *args, **kwargs):
        if request.user.is_authenticated and not models.MatrixUser.objects.filter(user=self.request.user.pk).exists():
